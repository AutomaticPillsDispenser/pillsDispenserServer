...
import { send } from "../routes/auth.js";
  ...
  try {
    ...
    if (existingUser) {
      if (!existingUser.verified) {
        ...
        const savedUser = await existingUser.save(); // Save the updated user information
                    ...
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px;">
                                ...
                                <table border="0" cellpadding="0" cellspacing="0">
                                  <tr>
                                    <td align="center" bgcolor="#1a82e2" style="border-radius: 6px;">
                                      <a href="${URL}/auth/verify/${existingUser._id}" target="_blank" style="display: inline-block; padding: 16px 36px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; color: #ffffff; text-decoration: none; border-radius: 6px;">Verify Email</a>
                                    </td>
                                  </tr>
                                </table>
                              </td>
                      ...
                      <tr>
                        <td align="left" bgcolor="#ffffff" style="padding: 24px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;">
                          ...
                          <p style="margin: 0;"><a href="${URL}/auth/verify/${existingUser._id}" target="_blank">${URL}/auth/verify/${existingUser._id}</a></p>
                        </td>
                      </tr>
...
export const validateLogin = async (
  ...
  try {
    ...
    const user = isEmail
      ? await User.findOne({ email: username.toLowerCase(), verified: true })
      : await User.findOne({ username, verified: true });
    if (!user) {
